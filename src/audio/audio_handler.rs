...
use crate::audio::decoder::{DecodedStream, decode_stream};
...
impl AudioHandler {
    pub fn mount() -> Self {
        ...
        std::thread::spawn(move || {
            ...
            let mut rb_producer: Option<
                ringbuf::wrap::caching::Caching<
                    Arc<rb::SharedRb<ringbuf::storage::Heap<f32>>>,
                    true,
                    false,
                >,
            > = None;
                ...
                if let Ok(event) = audio_rx.recv() {
                    match event {
                        ...
                        AudioCommand::HlsReceived(bytes) => {
                            let decoded_stream = AudioHandler::decode_segment(&bytes, 0)
                                .expect("should decode segment");
                            if let Some(prod) = rb_producer.as_mut() {
                                ...
                                let paused_second_clone = paused_clone.clone();
                                let volume_second_clone = volume_clone.clone();
                                let stream = device.build_output_stream(
                                    &stream_config,
                                    move |data: &mut [f32], _: &cpal::OutputCallbackInfo| {
                                        for sample in data.iter_mut() {
                                            if paused_second_clone.load(Ordering::Relaxed) {
                                                *sample = 0.0;
                                            } else {
                                                *sample = consumer.try_pop().unwrap_or(0.0) * f32::from_bits(volume_second_clone.load(Ordering::Relaxed));
                                            }
                                        }
                                    },
